...
\section{Raspberry Pi Safety and Usage}

The Raspberry Pi~5 is the compute brain of your robot.
A damaged or corrupted RPi means your whole team is blocked until it is
re-imaged or replaced --- which can take an entire lab session.
The failures we see most often are \emph{entirely preventable}.
Read this section before touching the hardware.

\subsection*{Power}

\begin{itemize}
  \item \textbf{Two valid power sources:}
        \begin{itemize}
          \item \textbf{Supplied USB-C PD adapter} (5\,V / 5\,A) --- use this
                at your bench when the robot battery is not connected.
          \item \textbf{Robot battery via the PCB} --- the PCB's onboard 5\,V
                rail is rated at 6\,A and includes a hot-swap circuit that
                safely switches between battery and bench supply.
                This is the intended power source when the robot is running
                autonomously.
        \end{itemize}
  \item \textbf{Never use a phone charger, laptop USB port, or cheap power bank.}
        These cannot supply enough current; the resulting voltage drop causes
        random crashes and SD card corruption.
  \item \textbf{Never wire the battery directly to the RPi}, bypassing the PCB.
        The PCB regulator and hot-swap circuit are there for a reason.
  \item \textbf{Never hot-swap the SD card.}
        If the card must be removed, shut down first (see below).
\end{itemize}

\subsection*{Shutdown procedure --- the most important habit}

The RPi runs a Linux filesystem.
Pulling the power while the OS is writing to the SD card --- which happens
constantly in the background --- \textbf{corrupts the filesystem}.
A corrupted SD card requires a full re-image and you lose all local work that
was not pushed to GitHub.

\medskip
\noindent\textbf{Always shut down properly before removing power:}

\begin{minted}{bash}
sudo shutdown -h now
\end{minted}

Wait for the green activity LED to stop blinking and go dark (roughly 10--15\,s),
\emph{then} unplug the power cable.
This also applies when you are finished for the day and when packing up the robot.

\subsection*{GPIO pins}

\begin{itemize}
  \item \textbf{GPIO logic level is 3.3\,V.}
        The RPi is \emph{not} 5\,V tolerant on its GPIO pins.
        Connecting a 5\,V signal directly to any GPIO pin will destroy that
        pin and may kill the entire SoC.
        Always use a level shifter or voltage divider when interfacing with
        5\,V peripherals.
  \item \textbf{Maximum current per GPIO pin is 16\,mA} (8\,mA recommended).
        Never drive an LED, motor, or relay directly from a GPIO pin without a
        current-limiting resistor or driver IC.
  \item \textbf{Never short GPIO pins together}, and keep metal objects (tools,
        loose wires, PCB offcuts) away from the board when it is powered.
        A momentary short across the wrong pins can instantly destroy the board.
  \item \textbf{Ground yourself} before handling the board.
        Touch a grounded metal surface (e.g.\ a PC chassis) or wear an ESD
        wrist strap.
        Electrostatic discharge is invisible and silent --- and permanently damages
        chips.
\end{itemize}

\subsection*{Physical handling}

\begin{itemize}
  \item Hold the board by its edges.
        Finger pressure on chips or connectors can crack solder joints.
  \item Keep the board on a \textbf{non-conductive surface} (the anti-static bag it came
        in, or a plastic tray).
        Placing it on a metal bench or a conductive surface shorts the bottom
        pads.
  \item Do not stack anything on top of the RPi when it is running.
        The processor generates significant heat; blocking airflow causes thermal
        throttling and instability.
\end{itemize}

\subsection*{SD card hygiene}

\begin{itemize}
  \item \textbf{Push your work to GitHub immediately.}
        SD cards have a finite write endurance and \emph{will} eventually fail.
        Any work that only exists on the RPi can vanish at any time (see also
        Task~1.3).
  \item Avoid running \texttt{apt upgrade} or long package installs when the
        robot is on battery --- a power cut mid-upgrade corrupts the package
        database and often requires a re-image.
  \item If the RPi fails to boot, do \emph{not} keep trying to power-cycle it.
        Note down any error messages on the HDMI output or status LEDs and ask
        the TAs for help.
\end{itemize}
...
\subsection{Task 1: Set Up Your Repository}\label{sec:task1}

\machinebox{\onlaptop\ your laptop \textbar\ \onrpi\ Raspberry Pi}{The repo must be cloned on \emph{both} machines. Your laptop uses it for firmware (Task~\ref{sec:task2}); the RPi uses it for ROS2 (Task~\ref{sec:task3}). Day-to-day edits are made on your laptop, then pulled onto the RPi.}
...
\subsubsection{Fork the Repository\onlaptop}
...
\subsubsection{Clone the Repository --- Laptop\onlaptop}
...
\subsubsection{Clone the Repository --- Raspberry Pi\onrpi}\label{ssec:clone-rpi}

SSH into the Raspberry Pi and clone the \emph{same fork} there.
The RPi copy is what Docker mounts when running the ROS2 workspace in Task~\ref{sec:task3}.
...
\subsubsection{Daily Git Workflow\onlaptop}
...
\subsubsection{Pull Updates from the Original Repository\onlaptop}
...
\subsection{Task 2: Flash the Firmware}\label{sec:task2}
...
\subsubsection{Prerequisites\onlaptop}
...
\subsubsection{Build and Upload\onlaptop}
...
\subsection{Task 3: Bring Up the ROS2 Environment and NUEVO UI}\label{sec:task3}

\machinebox{\onrpi\ Raspberry Pi (SSH or direct terminal)}{Most steps run on the RPi. The final step (Access the NUEVO UI) opens in a browser on any device on the same network. Make sure you have already cloned your fork on the RPi (Section~\ref{ssec:clone-rpi}) and run \texttt{git pull} to get the latest code before starting.}
...
\subsubsection{Raspberry Pi UART Setup (one-time)\onrpi}
...
\subsubsection{Install Node.js and npm (one-time)\onrpi}
...
\subsubsection{Build the Frontend (one-time)\onrpi}
...
\subsubsection{Install Docker (one-time)\onrpi}
...
\subsubsection{Start the Docker Container\onrpi}
...
\subsubsection{Launch the Bridge Node\onrpi}
...
\subsubsection{Access the NUEVO UI\onui}
...
\subsection{Task 4: DC Motor PI Tuning and Step Response}\label{sec:task4}

\machinebox{\onlaptop\ your laptop \textbar\ \onui\ NUEVO UI \textbar\ \onrpi\ RPi}{MATLAB analysis and gain calculations run on your laptop. Motor commands and data recording use the NUEVO UI in your browser. The RPi must be running the bridge node from Task~\ref{sec:task3}.}
...
\subsubsection{Model-Based PI Design (Optional)\onui\onlaptop}\label{ssec:model-pi}
...
\subsubsection{Simple Observation (Alternative)\onui}\label{ssec:simple-obs}
...
\subsubsection{Record and Compare Step Responses\onui\onlaptop}
...
\begin{enumerate}
  ...
  \item Your \textbf{tuned} gains from Section~\ref{ssec:model-pi} or~\ref{ssec:simple-obs}.
\end{enumerate}
