...
class DWAPlanner():
    ...
    def compute_velocity(self, path, pose, velocity, obstacles, dt): # all parameters should be on world frame!!!
        ...
        x, y, theta = pose
        if len(obstacles) > 0:
            # Transform obstacles to robot frame and filter out those beyond the obstacle range to reduce computational load and focus on nearby threats, ensuring that the planner prioritizes immediate obstacles while ignoring distant ones that are less relevant for immediate navigation decisions.
